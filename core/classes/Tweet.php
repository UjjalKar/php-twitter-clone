<?php  
	...
	class Tweet extends User {
		...
		public function tweets($user_id) {
			$stmt = $this->pdo->prepare("SELECT * FROM `tweets` LEFT JOIN `users` ON `tweetBy` = `user_id` WHERE `tweetBy` = :user_id AND `retweetID` = '0' OR `tweetBy` AND `retweetBy` != :user_id");
			...
			foreach ($tweets as $tweet) {
				...
				<div class="all-tweet">
					...
					'. (($retweet['retweetID'] === $tweet->retweetID OR $tweet->retweetID > 0) ? '
						...
						'.((!empty($tweet->retweetMsg) && $retweet['tweetID'] or $tweet->retweetID > 0) ? '
							...
							<div class="t-show-head">
								...
								<div class="t-s-head-content">
									<div class="t-h-c-name">
										<span><a href="'.BASE_URL.$user->username.'">Retweet '.$user->screenName.'</a></span>
						...
						<div class="t-show-footer">
							<div class="t-s-f-right">
								<ul> 
									...
									<li>'.(($likes['likeOn'] === $tweet->tweetID) ? '<button class="unlike-btn" data-tweet="'.$tweet->tweetID.'" data-user="'.$tweet->tweetBy.'"><a href="javascript:void(0)"><i class="fa fa-heart" aria-hidden="true"></i></a><span class="likesCounter">'.(($tweet->likesCount) === 0 ? '' : $tweet->likesCount).'</span></button>' : '<button class="like-btn" data-tweet="'.$tweet->tweetID.'" data-user="'.$tweet->tweetBy.'"><a href="javascript:void(0)"><i class="fa fa-heart-o" aria-hidden="true"></i></a><span class="likesCounter">'.(($tweet->likesCount) === 0 ? '' : $tweet->likesCount).'</span></button>').'</li>
										<li>
		...
		public function getTrendByHash($hashtag) {
			...
			// % is wildcard operator.
			$stmt->bindValue(':hashtag', $hashtag.'%');
			$stmt->execute();
			return $stmt->fetchAll(PDO::FETCH_OBJ);
		}

		public function getMenstion($mention) {
		...
		public function getTweetLinks($tweet) {
			...
			$tweet = preg_replace("/#([\w]+)/", "<a herf='".BASE_URL."hashtag/$1'>$0</a>", $tweet);
			$tweet = preg_replace("/@([\w]+)/", "<a herf='".BASE_URL."$1'>$0</a>", $tweet);
			return $tweet;
		}
		...
		public function addLike($user_id, $tweet_id, $get_id) {
			...
			$stmt = $this->pdo->prepare("SELECT * FROM `tweets` WHERE `tweetID` = :tweet_id");
			$stmt->bindParam(":tweet_id", $tweet_id, PDO::PARAM_INT);
			$stmt->execute();
			return $stmt->fetchAll(PDO::FETCH_OBJ);
	...
	'</div>
						<div class="t-show-footer">
							<div class="t-s-f-right">
								<ul> 
									<li><button><a href="#"><i class="fa fa-share" aria-hidden="true"></i></a></button></li>	
									<li><button class="retweet" data-tweet='.$tweet->tweetID.' data-user='.$tweet->tweetBy.'><a href="javascript:void(0)"><i class="fa fa-retweet" aria-hidden="true"></i></a></button></li>
									<li>'.(($likes['likeOn'] === $tweet->tweetID) ? '<button class="unlike-btn" data-tweet="'.$tweet->tweetID.'" data-user="'.$tweet->tweetBy.'"><a href="javascript:void(0)"><i class="fa fa-heart" aria-hidden="true"></i></a><span class="likesCounter">'.(($tweet->likesCount > 0) ? $tweet->likesCount : '').'</span></button>' : '<button class="like-btn" data-tweet="'.$tweet->tweetID.'" data-user="'.$tweet->tweetBy.'"><a href="javascript:void(0)"><i class="fa fa-heart-o" aria-hidden="true"></i></a><span class="likesCounter">'.(($tweet->likesCount > 0) ? $tweet->likesCount : '').'</span></button>').'</li>
										<li>
										<a href="#" class="more"><i class="fa fa-ellipsis-h" aria-hidden="true"></i></a>
										<ul> 
