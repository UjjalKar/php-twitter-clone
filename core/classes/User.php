<?php 
	...
	class User {
		...
		public function checkPassword($password) {
			...
			$stmt->bindParam(":password", $enPassword, PDO::PARAM_STR);
			$stmt->execute();
		...
		public function uploadImage($file) {
			...
			if(in_array($ext, $allowed_ext) === true) {
				if($error === 0) {
					if($fileSize <= 419430400000 ) {
						$fileRoot = 'users/' . $filename;
						move_uploaded_file($fileTmp, $fileRoot);
						return $fileRoot;

					} else {
						$GLOBALS['imageError'] = "The file is greater than 2mb";
					}
				}
			} else {
				$GLOBALS['imageError'] = "The extension isn't supported";
			}
		}
	}